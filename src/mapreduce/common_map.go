...
func doMap(
  ...
  //create intermediate files and write kv in json format
  kvs := mapF(inFile, string(contents))
  interFiles := make([]*os.File, nReduce) 
  interEnc := make([]*json.Encoder, nReduce)

  for i := 0; i < nReduce; i++ {
    tmpFile, err := os.Create(reduceName(jobName, mapTaskNumber, i))
    if err != nil {
      log.Fatal(err)
    }
    interEnc[i] = json.NewEncoder(tmpFile) //create encoder which writes to each file
    interFiles[i] = tmpFile
  }
  for _, kv := range kvs {
    err := interEnc[ihash(kv.Key) % nReduce].Encode(&kv)
    if err != nil {
      log.Fatal(err)
    }
  }
